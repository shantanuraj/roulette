...
- Keys already encode temporal information; no separate metadata store is
  required
- URL-driven constraints keep the service stateless
- Filtering logic is shared between offline tooling and runtime behavior
- The API remains composable (`latest` + `after`)
...
| `PORT`               | no       | Port to bind the HTTP server (default: `3000`)         |
